...
describe('blackSwanRepo', () => {
  ...
  it('scenarios CRUD', async () => {
    const { createBlackSwanScenario, deleteBlackSwanScenario, listBlackSwanScenarios } = await import('./blackSwanRepo')
  ...
  it('runs persistence roundtrip', async () => {
    const { saveBlackSwanRun, getLastBlackSwanRun } = await import('./blackSwanRepo')
    await saveBlackSwanRun({ ts: Date.now(), horizon: 7, sims: 500, seed: 42, weightsSource: 'manual', mix: 0, summary: { pRed7d: 0.2, esCollapse10: 0.3, sirenLevel: 'amber', probEverRed: 0.2, probThresholdEnd: 0.1, esCoreIndex: 4 }, payload: { days: [1], coreIndex: { p10: [1], p50: [2], p90: [3] }, pCollapse: { p10: [0.1], p50: [0.2], p90: [0.3] }, histogram: [{ bucket: '0-1', value: 2 }], tail: { probEverRed: 0.2, probThresholdEnd: 0.1, probThresholdEver: 0.3, esCoreIndex: 4, esCollapse: 0.3, coreIndexTail: { alpha: 0.9, var: 4, es: 3.7, tailMean: 3.7, tailMass: 0.2, n: 10, method: 'linear-interpolated', warnings: [] }, collapseTail: { alpha: 0.9, var: 0.28, es: 0.3, tailMean: 0.3, tailMass: 0.2, n: 10, method: 'linear-interpolated', warnings: [] } }, topDrivers: [], recommendations: [], noteRu: 'ok' } })
